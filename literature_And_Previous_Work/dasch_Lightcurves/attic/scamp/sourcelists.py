...
def save_ds9reg(filename,ra,dec,errors=1.,minerror=0.05,color='green',width=1,text='',tag='',header='fk5'):
    """
...
def read_ds9reg(filename,shape='circle',radius=True):
    """
    Save ra and dec as DS9 region file
	...
	if shape in line:
	    ...
	    #print coords
	    if ':' in coords[0]:
		rai = hms2deg(coords[0])
	    else:
		rai = float(coords[0])
	    if ':' in coords[1]:
		deci = dms2deg(coords[1])
	    else:
		deci = float(coords[1])
	    ra.append(rai)
	    dec.append(deci)
...
def deg2hms(RAin,separator=':',plus='+'):
    if isarray(RAin):
	result=[]
	for RAini in RAin:
...
def _deg2hms(RAin,separator=':',plus='+'):
   ...
   if(sign == -1):
      out = ('-%02d'%h)+separator+('%02d'%m)+separator+('%06.3f'%s)
   else: out = plus+('%02d'%h)+separator+('%02d'%m)+separator+('%06.3f'%s)

   return out

# Convert Decl. (deg) to D.M.S:
def deg2dms(Decin,separator=':',plus='+'):
    if isarray(Decin):
	result=[]
	for Decini in Decin:
...
def _deg2dms(Decin,separator=':',plus='+'):
   ...
   if(sign == -1):
      out = ('-%02d'%d)+separator+('%02d'%m)+separator+('%06.3f'%s)
   else: out = plus+('%02d'%d)+separator+('%02d'%m)+separator+('%06.3f'%s)
...
def match2(ra1,dec1,ra2,dec2,errors='none',radius=.2,onlybest=False,bsra=0,bsdec=0):
    """
    ...
    pts=np.array(zip(ra2*np.cos(decmd_rad),dec2))
...
#----------
def match2_boresight(ra1,dec1,ra2,dec2,errors,bsra_init=0.,bsdec_init=0.):
    """
    ...
    - ra1, dec1: first source list with RA and Dec in degrees
    - ra2, dec2: second source list with RA and Dec in degrees
    - errors: error in arcsec for the most distant match for each source in the second list
    - bsra_init: initial RA correction in arcsec for the second source list
    - bsdec_init: initial Dec correction in arcsec for the second source list
    ...
    # match source lists
    ind1,ind2,dist12 = match2(ra1, dec1, ra2+bsra_init_shift, dec2+bsdec_init/3600., errors=errors, onlybest=True)
    # weighted mean for RA and Dec
    nxi = len(ind2)
    wi = (1./errors[ind2]**2)
    sumwi = np.sum(wi)
    radist = (ra1[ind1] - (ra2[ind2]+bsra_init_shift))
    bsra_shift = np.sum(wi*radist) / sumwi
    ...
    bsrasig_shift = np.sqrt(np.sum(wi*(radist-bsra_shift)**2) / ((nxi-1)*sumwi))
...
#----------
def match2_boresight_loop(ra1,dec1,ra2,dec2,errors, bsra=0.,bsdec=0.,bserr=.4,precision=0.001):
    """
	...
	ind1,ind2,dist12,bsra,bsdec,bsrasig,bsdecsig,nxi = match2_boresight(ra1, dec1, ra2, dec2, np.sqrt(errors**2+bserr**2), bsra_init=bsra_tot, bsdec_init=bsdec_tot)
	bsra_tot += bsra*0.8
	bsdec_tot += bsdec*0.8
...
#----------
def match2_boresight_plot(ra1,dec1,ra2,dec2,errors, maxsh=5., incr=.5, bsra=0.,bsdec=0.,bserr=.4,precision=0.001):
    """
    Iterate to get boresight correction with given precision
    ...
    - bsrasig, bsdecsig: 1 sigma error on boresight correction
    """
    shifts=np.array(range(2*maxsh/incr+1))*incr-maxsh
    bsra_tot=bsra
    bsdec_tot=bsdec
    bsra=1
    bsdec=1
