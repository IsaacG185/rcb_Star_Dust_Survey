...
if ($#argv == 1) then
  ...
  if ($#argv == 2) then
     ...
     echo "usage: ingest_colorterm.csh list [qualifier]"
...
echo "Writing: ${listfile:t:r}_colorterm.db"

if (-e ${listfile:t:r}_colorterm.db) then
  rm ${listfile:t:r}_colorterm.db
endif
...
#  echo "series1 is $series1 series $series plateNumber $plateNumber"
  ...
  set colortermtxt = ${plate:t}${qualifier}.colorterm.txt
  ...
  set catalog = `ls ${plate}${qualifier}_a?.colorterm.txt` 
  if ($#catalog > 0) then  

    foreach catalog ( ${plate}${qualifier}_a?.colorterm.txt ) 
      @ sbin = $sbin + 1 
    end
    ...
    foreach bin (1 2 3 4 5 6 7 8 9)
      set catalog = "${plate}${qualifier}_a${bin}.colorterm.txt"
      if (-e $catalog) then
          @ nbin = $nbin + 1
    ...
    paste spatial_bin_definition${listroot}${qualifier}.tmp para${listentry}${qualifier}.2.tmp | gawk '{OFS="\t"}{print $1,$2,$3,$4,$5,$6,$7}' | column $headerflag  >>  ${listfile:t:r}_colorterm.db
