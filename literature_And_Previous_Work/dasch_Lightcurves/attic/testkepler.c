...
/* testkepler.c
 *
 *  Test access to the Kepler field program. Adaped by RA2fpix by David Koch  D.Koch@nasa.gov, version of Wed, 17 Jun 2009
...
int main(int argc,char *argv[])
{
  ...
  double yreverse;
  double tempreverse;
  double maxXreverse = 0;
  double maxYreverse = 0;
  int offscl;
  int offsclCount = 0;
  int insclCount = 0;
#endif /* DEBUG_TNX */
  ...
  if (idtable_name[0] == 0) {
    fprintf(stderr,"ERROR: No idtable filename was specified\n");
    errorFlag = 1;
  }

  idtable_handle = Open(idtable_name,"r");
  ...
  if (errorFlag) {
    fprintf(stderr,"Usage: testkepler -i <idtable file> \n");
  ...
  printf("testkepler of %s %s\n",
         __DATE__,__TIME__);


  fprintf(outHandle,"REF\tra\tdec\tchn\trow\tcol\n");
  fprintf(outHandle,"---\t--\t---\t---\t---\t---\n");
  ...
  idtable_descriptor = table_create_descrip(&idtable_nrecs,
                                            ...
                                            TblDbl,"dec"  ,TblOff(PIDTABLE,dec),
                                            TblBuf,"REF"    ,TblOff(PIDTABLE,REF),MAX_REF,
  ...
  while (1) {
    ...
    result = RA2fpix(pIdTable->ra,pIdTable->dec,&chn_n,&p_row,&p_coln);
    if (result) {
      output_recs++;
      fprintf(outHandle,"%s\t%.6f\t%.6f\t%d\t%.2f\t%.2f\n",
              pIdTable->REF,
              pIdTable->ra,
              pIdTable->dec,
              chn_n,
  ...
  if (verbose) {
    fprintf(stderr,"read %d records for %s\n",idtable_nrecs,idtable_name);
  }

  /* We are done with the idtable table.  Clear memory */
  if (idtable_header != NULL) {
    table_hdrfree(idtable_header);
    idtable_header = NULL;
  }
  if (idtable_handle != NULL) {
    Close(idtable_handle);
    idtable_handle = NULL;
  }
  if (idtable_descriptor != NULL) {
    Free(idtable_descriptor);
    idtable_descriptor = NULL;
  }
  if (idtable_row != NULL) {
    table_rowfree(idtable_row);
    idtable_row = NULL;
  }
  ...
  time(&curTime);
  curTime -= startTime;
  fprintf(stdout,"testkepler stars in: %d stars out: %d seconds %d for %s\n",
          idtable_nrecs,
          output_recs,
          curTime,
          outfile);
