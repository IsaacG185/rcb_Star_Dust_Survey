...
%   $Revision: 1.15 $  $Date: 2002/03/19 21:25:08 $
...
l(arg2) = -inf;
if any(u == l) 
   errmsg=sprintf('Equal upper and lower bounds not permitted.');
   error(errmsg)
elseif min(u-l) <= 0
...
if strcmp(optimget(options,'DerivativeCheck',defaultopt,'fast'),'on')
   warnstr = sprintf(['Trust region algorithm does not currently check user-supplied derivatives,\n',...
...
while ~ex & ~isequal(OPT_STOP, 1) 
   if any(~isfinite(fvec)) 
      errmsg= sprintf('%s cannot continue: user function is returning Inf or NaN values.', caller);
      error(errmsg)
   end
...
if OPT_STOP
    error('Fitting computation cancelled.');
end
%
%   Display
if showstat > 1 
