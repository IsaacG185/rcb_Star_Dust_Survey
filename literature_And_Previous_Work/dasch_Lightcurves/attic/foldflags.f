...
* Folding program to fold without binning.
	...
	PARAMETER( limit=1e6)
	double precision Z,P,cycle, T(1:limit),F(1:limit),E(1:limit),PHASE(1:limit)
	double precision FLAG(1:limit)
...
c	print*,' ( 0 for first datapoint)'
	...
	do i=1,limit
		read(1,*,end=2,err=2) T(i),F(i),E(i),FLAG(i)
	enddo
2	points=i-1
...
C omitt the errors on the second cycle.
	do 22 i=1,points*2
         	if ( i.gt.points ) then
			...
			T(i)=T(i-points)
			E(i)=0.0d+00
 			FLAG(i)=FLAG(i-points)
		endif
22	continue

	do i=1,points*2
		write(*,99) PHASE(i),T(i),F(i),E(i),FLAG(i)		
	enddo

99	format (4X,F10.6,4X,F18.6,4X,F10.6,4X,G12.6,4X,G12.6)
