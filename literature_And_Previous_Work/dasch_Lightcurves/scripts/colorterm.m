...
for kkk=1:ksize
  ...
  % maintain their own random state, so you can't (safely) just seed one. We
  % only use a couple functions here, but for copy-paste friendliness, we
  ...
  % Compute a colorterm for each bin
  ...
  for bin=1:9
    ...
    % Initial filtering for reasonable-looking records:
    ...
    % Filter out 3-sigma outliers in terms of isomag vs. stdmag
    if (length(ix) > 0)
      stdmag0 = stdmag(ix);
      iso0 = iso(ix);
      color0 = color(ix);

      [stdmag, ir] = sort(stdmag0);
      iso = iso0(ir);
      color = color0(ir);
      ...
      diff2 = (iso - isos).^2; % squared difference between smoothed and actual isomags
      rms2s = std(diff2);    % standard deviation of rms^2
      ix = find(diff2 < 3^2 * rms2s);  % remove 3-sigma ouliters
    end

    count2 = length(ix);

    % Second round of removing 3-sigma outliers
    if (length(ix) > 0)
      iso3 = iso(ix);
      iso3s = isos(ix);
      stdmag3 = stdmag(ix);
    ...
    if (length(ix) > 0)
      ...
      diff2 = (iso3 - iso3s).^2;
      rms3s = std(iso3 - iso3s)^2;
      ix = find(diff2 < 4^2 * rms3s);
    end

    count4 = length(ix);

    % Derive a rough limiting magnitude and filter out stars less than 4 sigma above it
    if (length(ix) > 0)
      iso4 = iso3(ix);
      iso4s = iso3s(ix);
      stdmag4 = stdmag3(ix);
      ...
      diff2 = (iso4 - iso4s).^2; % squared difference between smoothed and actual isomags
      diff2s = smooth(stdmag4, diff2, 0.2); % smoothed diff2 - basically noise variance (mag^2 units) as function of stdmag
      dist2 = (iso4 - max(iso4s)).^2; % squared difference between isomags and the faintest smoothed isomag
      dist2s = smooth(stdmag4, dist2, 0.2); % smoothed dist2
      ix = find(diff2s > dist2s / 16); % 4^2; unlike above this `ix` IDs stars that are REJECTED

      % To investigate: sometimes the above filter rejects nearly all of our
      % stars, even when we have thousands of them. Seems like that shouldn't
      % happen? E.g. sometimes mean(diff2) = 1.1 and mean(dist2) = 1.2; the
      % latter seems totally bogus.
    end
    ...
    printf(
      "bin %d star counts: %d %d %d %d %d (%d) %d\n",
      bin, rows(input), count1, count2, count3, count4, count5, size(stdmag(:),1)
    );

    if size(stdmag(:),1) <= 100
      printf("Not enough stars to solve bin %d\n", bin);
    else
      ...
      stdmag2=stdmag(ix);
      color2=color(ix);
      stdmags2=interp1(isos,stdmag,iso2,'cubic','extrap');
      ...
      if abs(coef)<0.5;
        ...
        % do a correlation to determine the second cterm value
        ...
        iso2=iso(ix);
        refmag2=refmag(ix);
        refmags2=interp1(isos,refmag,iso2,'cubic','extrap');
      ...
      if (length(ix) > 2)
        ...
        xdata=cterm(ix);
        ydata=myrms(ix);
        f = @(c, x) c(1)*(x-c(2)).^2+c(3); % inline('c(1)*(x-c(2)).^2+c(3)','c','x');
    ...
    clear covcolor    errcolor    iso2        k2          mincterm    refmag      stdmags2
    clear REF         cterm       exitflag    iso3        kdE         minrms      refmag2     diff2s       stdpointa
    clear c           cterm0      extinction  iso3s       keff        myrms       refmags2    rmsout      x
    clear coef        ddec        f           iso4                    myrms0      resi        sigcolor    x0
    clear color       dec         flag        iso4s       kleft       dist2       sigma       x_image
    clear color0      dfi         flags       iso5s       km          norm        dist2s      stdmag      xdata
    clear color2      dmag        h           isopointa   kright      number      diff2       stdmag0     xfit
    clear color3      dmagout     ir          isos        output      rms2s       stdmag2     y_image
    clear color4      dmyrms      iso         ix          lambda      p           stdmag3     ydata
