...
#
#  search_close3.csh - Look for close multiple exposures
#
#  Mar 25, 2020 Edward J. Los - Adapted from search_close3.csh
#
if ($#argv != 2) then
    echo "usage: search_close3.csh <solutionNumber> <list>"
    exit
endif
#if ($#argv != 2) then
#    echo "usage: search_close3.csh <list> <analysis_threshold>"
#    exit
#endif
...
foreach plate ( `cat ${DASCH_SCRIPTS}/$listroot` ) 
  ...
  set datestr = `date`
  echo "Starting search_close3 on $datestr"
  ...
  set outputname = ${DASCH_MATCH}/${plate}_close3.db
  ...
  if (-e ${DASCH_MATCH}/${plate}_tnx.db) then
    ...
    if (-e ${DASCH_MATCH}/${plate}.db) then
       ...
       echo "ERROR: search_close3 found no sextractor file for $plate"
       continue
    endif
  endif
...
#       echo "ERROR getlocation failed with args -a $image"
  ...
  if (-e $inputname) then
    echo "$DASCH_SCRIPTS/search_close3  -v $plotstr -t $analysis_threshold  $calibrationSquare  -s $platescale -w $width -h $height -g /dasch/raid021/junk/plates -r ${plate} -i $inputname -o $outputname"
    #echo "ERROR: Early exit"
    #exit
    $DASCH_SCRIPTS/search_close3   $plotstr  -t $analysis_threshold  $calibrationSquare -w $width -s $platescale -h $height -g /dasch/raid021/junk/plates -r ${plate} -i $inputname -o $outputname
