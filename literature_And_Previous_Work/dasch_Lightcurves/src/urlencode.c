 ...
 *
 * Jun  8, 2010 Edward J. Los Ported from http://aolserver.am.net/cvs/aolserver/aolserver/nsd/urlencode.c
 *
 *
 *   To test: cc -g -O0 -DURLDEBUG urlencode.c -o urlencode
 */
...
#include <stdarg.h>
#include <string.h>
static const char *RCSID = "@(#) $Header: /cvsroot/aolserver/aolserver/nsd/urlencode.c,v 1.17 2005/03/25 00:39:47 jgdavidson Exp $, compiled: " __DATE__ " " __TIME__;
...
char *urlencode(char *string, char *outstring, int outlength)
{
  ...
  n++;
  if (n >= outlength) {
    printf("ERROR: buffer length %d for urlencode is too small for %s, need %d\n",outlength,string,n);
    exit(-1);
  }
...
char *urldecode(char *string, char *outstring,int outlength)
{
  register int i, j, n;
  register char *p, *q;
  char c;
  ...
  while ((c = (unsigned char)(*p)) != '\0') {
    if (c == '%' && n > 2 &&
	(i = enc[(unsigned char)(p[1])].hex) >= 0 &&
	(j = enc[(unsigned char)(p[2])].hex) >= 0) {
...
main(int argc,char *argv[])
{
  int index;
  char inString[257];
  char outString[4*256];
  char copyString[4*256];
  for (index = 0; index < 255; index++) {
    inString[index] = index+1;
  }
  urlencode(inString,outString,4*256);
  printf("outString %s\n",outString);
  urldecode(outString,copyString,4*256);
  for (index = 0; index < 255; index++) {
    if (copyString[index] != inString[index]) {
      printf("ERROR: index %d, copyString %d inString %d\n",index,copyString[index],inString[index]);
    }
  }
  for (index = 32; index < 128-1; index++) {
    inString[index-32] = index;
  }
  inString[128-1-32] = 0;

  urlencode(inString,outString,4*128);
