...
%   $Revision: 1.12 $  $Date: 2002/03/19 21:25:06 $
...
%         SD=-(GRAD*GRAD'+(norm(GRAD)+1)*(eye(numberOfVariables,numberOfVariables)))\(GRAD*CostFunction);
      ...
      elseif NewF<FIRSTF
         ...
         if ~levMarq
            if currstepsize>1e-8 & cond(JAC)<1e8
               ...
               if verbosity > 0
                  warning('Conditioning of Gradient Poor - Switching To Levenberg-Marquardt method.')
               end
               how='CHG2LM';
               levMarq=1;
   ...
   case 'fun_then_grad'
      ...
      numGradEvals=numGradEvals+1;
   otherwise
      error('Undefined calltype in CFLSQCURVEFIT');
   end
   numFunEvals=numFunEvals+1;
